...
io.on('connection', (socket) => {
  ...
  socket.on('start_exam', async (sessionId) => {
    console.log(`[Socket Event: start_exam] Received for session ID: ${sessionId}`);
    try {
        const session = await ExamSession.findById(sessionId);
        if (!session) {
            console.error(`[Socket Event: start_exam] Session with ID ${sessionId} not found.`);
            return;
        }
        console.log(`[Socket Event: start_exam] Found session:`, session);
        ...
        console.log(`[Socket Event: start_exam] Created attempt map:`, attemptMap);

        // 4. Emit a personalized 'exam_started' event to each socket in the lobby
        const socketsInRoom = await io.in(sessionId).fetchSockets();
        console.log(`[Socket Event: start_exam] Found ${socketsInRoom.length} sockets in room ${sessionId}.`);
